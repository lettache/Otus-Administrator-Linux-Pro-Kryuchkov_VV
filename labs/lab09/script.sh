...
{
  {
    echo "Список запрашиваемых адресов с наибольшим кол-вом запросов"
    awk -v timestamp="$timestamp" '
      function monStrToInt(mon){
        m=split("Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec",d,"|")
        for(o=1;o<=m;o++)
...
mail -s "Отчет о работе nginx otus@localhost < $mailfile
rm -f "$lockfile"; exit $?